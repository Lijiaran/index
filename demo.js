window.onload=function () {
    ...
    function animate(offset) {
    	...
    	var go=function() {
    		if(speed > 0 && parseInt(list.style.left) < left || speed < 0 && parseInt(list.style.left) > left) {           //speed>0 代表向左移动必然目标位置left大于初始位置style.left 这里
		        ...
		        if(left<-3000) {                      //判断是否到达空白图片，这里每张图片的width为600px  left<-3000 说明是要从第五张图片跳到第一张图 就要把它初始化为的第一张图的位置
		        	list.style.left="-600px";          
		        }
		        if(left>-600) {                        //判断是否到达空白图片，这里每张图片的width为600px  left>-600 说明是要从第一张图片跳到第五张图 就要把它初始化为的第五张图的位置
	...
	for (var i = 0; i < buttons.length; i++) {           
		buttons[i].onclick=function() {              
		    ...
		    var offset = -600 * (myIndex-index);                                 //偏移量   目标值减去当前值
		    animate(offset); 
